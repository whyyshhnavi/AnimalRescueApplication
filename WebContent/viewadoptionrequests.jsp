			...
			%>
					<%
						for (ServiceRequest serviceRequest : HibernateDAO.getServiceRequests()) {
							...
							if (serviceRequest.getUserid().equals(username) && serviceRequest.getRequesttype().equals("adoption")) 
							{
					%>
								<div class="about-sub">
									<div class="col-md-6 about_bottom_left">
										...
										<a class="button-style" href="viewadoptionrequests.jsp?did=<%=serviceRequest.getId()%>">delete</a>
										<a class="button-style" href="viewadoptionrequests.jsp?did=<%=serviceRequest.getId()%>">is completed?</a>
									</div>
									<!-- Stats-->
			...
			%>
					<%
						for (ServiceRequest serviceRequest : HibernateDAO.getServiceRequests()) {
							...
							if (serviceRequest.getStatus().equals("inprocess") && serviceRequest.getRequesttype().equals("adoption")) 
							{
	...
	<%
		...
		if (uid != null) {
			...
			if (HibernateTemplate.updateObject(serviceRequest) == 1) {
				response.sendRedirect("viewadoptionrequests.jsp?status=success");
			} else {
				response.sendRedirect("viewadoptionrequests.jsp?status=failed");
			}
		}
	%>
